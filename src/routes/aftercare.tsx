  ...
  <>
    ...
    <article className="pt-40 pb-32">
            ...
            {aftercare.after.phases.map((p, i) => (
              <li key={p.label} className="reveal grid grid-cols-12 gap-6">
                <div className="col-span-12 md:col-span-3">
                  ...
                  <div className="display mt-1 text-2xl text-(--bone-paper)">
                    {p.label}
                  </div>
                </div>
      ...
      <section className="mx-auto mt-32 max-w-350 px-6 md:px-10">
        <div className="border border-(--blood)/40 bg-(--blood)/[0.06] p-8 md:p-12">
          ...
          <ul className="grid gap-3 md:grid-cols-2">
            {aftercare.after.donts.map((d) => (
              <li
                key={d}
                className="serif-tight flex gap-3 text-lg text-(--bone-warm)"
              >
                <span aria-hidden className="text-(--blood-bright)">×</span>
                <span>{d}</span>
              </li>
            ))}
          </ul>
